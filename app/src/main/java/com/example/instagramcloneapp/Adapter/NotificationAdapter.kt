...
class NotificationAdapter(
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        //Display user's information on the notification fragment.
        userInfo(holder.profileImage, holder.userName, notification.getUserId())

        //If it is a post i.e if someone liked a post and commented on a post, then we have to display postImage as well.
        ...
        //Go to a post/ profile on getting a notification.
        holder.itemView.setOnClickListener {
            if(notification.isIsPost()){
                ...
                editor.apply()
                (mContext as FragmentActivity).supportFragmentManager.beginTransaction()
                    .replace(R.id.fragment_container, ProfileFragment()).addToBackStack("profile ${notification.getUserId()}").commit()
            }
        }
    }
